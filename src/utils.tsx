...
export const addItalicToLi = (t: string, index: number): JSX.Element => {
  ...
  if (t.includes("(")) {
    ...
    if ((t.match(new RegExp("\\(", "g")) || []).length === 2) {
      ...
      return <li key={index}>- <><i>{arr[0]}</i>{t.slice(posOfRightParenthesis + 1, lastPosOfLeftParenthesis)}
        <i>{arr[1]}</i></></li>;
    }
    return <li key={index}>- {posOfLeftParenthesis === 0 ? (<>
        <i>{arr[0]}</i>{t.slice(posOfRightParenthesis + 1, t.length)}</>) :
